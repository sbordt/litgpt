...
if __name__ == "__main__":
    ...
    parser.add_argument("--layernorm_no_bias", action="store_true", default=False, help="disable bias for LayerNorm")
    ...
    model_config.layernorm_bias = not args.layernorm_no_bias
    ...
    parameter_metrics_spec = {
        ...
        r".*(norm_.*.weight|ln_f.weight).*": {"opnorm": lambda param: param.abs().max(dim=-1).values},        # operator norm for normalization layers (the maximum parameter value)
        r".*(mlp\.(fc|proj)\.weight|lm_head.weight).*" : {"opnorm": matrix_opnorm},            # operator norm for linear layers   
    }
    parameter_difference_metrics_spec = {
        ...
        # operator norm of the weight updates (for linear and normalization layers)
        r".*(norm_.*.weight|ln_f).*": {"opnorm": lambda param, ref_param: (param-ref_param).abs().max(dim=-1).values},   
        r".*(mlp\.(fc|proj)\.weight|lm_head.weight).*" : {"opnorm": lambda param, ref_param: matrix_opnorm(param-ref_param)},                        
    }
    gradient_metrics = {
        "l2norm": l2_norm,
    }
    ...
    # debug logging level
    # logging.getLogger("ModuleMonitor").setLevel(logging.INFO)
